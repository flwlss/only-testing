...
export const rotateAnimation = (slideIndex: number) => {
  const rotateDeg = 360 / keyEventsLength * (-slideIndex + 1)
  ...
  gsap.to(`.circlePagination__dot:nth-child(${slideIndex})`, { scale: 1, border: '1px solid rgba(48, 62, 88, 0.5)', background: '#F4F5F9', duration: 0 })
}